...
export default function AddEditProductScreen({ route, navigation }) {
  ...
  const remove = () => {
    ...
    if (Platform.OS === "web") {
      const confirmou = window.confirm("Deseja remover este produto?");
      if (confirmou) {
        deleteProduct(editing.id, (err) => {
          if (!err) navigation.goBack();
          else Alert.alert("Erro", "Não foi possível remover o produto.");
        });
      }
    } else {
  ...
  return (
    <View style={{ flex: 1, padding: 12 }}>
      ...
      {editing && (
        <Button title="Remover produto" onPress={remove} color="#c00" />
      )}
    </View>
  );
}
